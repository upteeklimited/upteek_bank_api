...
def login_with_email(db: Session, email: str=None, password: str=None, fbt: str=None):
    try:
        user = get_single_user_by_email_and_user_type(db=db, email=email, user_type=USER_TYPES['bank']['num'])
        if user is None:
            return {
                'status': False,
