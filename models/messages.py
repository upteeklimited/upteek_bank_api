...
def create_message(db: Session, sender_user_id: int = 0, receiver_user_id: int = 0, previous_message_id: int = 0, reference: str = None, title: str = None, body: str = None, attached_file: str = None, status: int = 0, commit: bool=False):
    message = Message(sender_user_id=sender_user_id, receiver_user_id=receiver_user_id, previous_message_id=previous_message_id, reference=reference, title=title, body=body, attached_file=attached_file, status=status, created_at=get_laravel_datetime(), updated_at=get_laravel_datetime())
    db.add(message)
    if commit == False:
        db.flush()
    else:
