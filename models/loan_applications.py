...
def get_single_loan_application_by_id(db: Session, id: int=0):
    return db.query(LoanApplication).options(joinedload(LoanApplication.financial_product), joinedload(LoanApplication.loan)).filter_by(id = id).first()
...
def get_loan_applications(db: Session, filters: Dict={}):
    query = db.query(LoanApplication).options(joinedload(LoanApplication.financial_product), joinedload(LoanApplication.loan))
